...
namespace ReusableModules.MetaProgramming;
...
public static class DynamicCopy<T> where T : struct
{
	...
	static DynamicCopy()
	{
		...
		// Reusable array index. We don't declare this until we know we need it
		LocalBuilder? i = null;

		foreach (FieldInfo fi in allFields)
		{
			if (fi.FieldType.IsPrimitive || fi.FieldType.IsEnum || fi.FieldType == typeof(string))
			{
			...
			else if (fi.FieldType.IsSZArray)
			{
...
public static class DynamicCopyClass<T> where T : class
{
	//private struct 

	public static readonly Action<T, T> IlEmitCopy;

	static DynamicCopyClass()
	{
		Type paramAsRefType = typeof(T);
		//Type paramAsRefType = paramType.MakeByRefType();
		FieldInfo[] allFields = paramAsRefType.GetFields(Constants.BINDING_FLAGS_FOR_ALL_FIELDS);

		MethodAttributes methodAttr = MethodAttributes.Public | MethodAttributes.Static;

		DynamicMethod dm = new("DeepCopyFromLhsToRhs", methodAttr, CallingConventions.Standard, null, new Type[] { paramAsRefType, paramAsRefType }, paramAsRefType.Module, true);
		ILGenerator il = dm.GetILGenerator();

		// Array index. We don't declare this until we know we need it
		LocalBuilder? i = null;

		//void DeepCopyStep(FieldInfo fi, int layer)
		//{
		//	if (fi.FieldType.IsPrimitive || fi.FieldType.IsEnum || fi.FieldType == typeof(string))
		//	{
		//		il.Emit(OpCodes.Ldloc, (layer * 2) + 1);
		//		il.Emit(OpCodes.Ldloc, layer * 2);
		//		il.Emit(OpCodes.Ldfld, fi);
		//		il.Emit(OpCodes.Stfld, fi);
		//	}
		//	if (fi.FieldType.IsSZArray)
		//	{
		//		Label exitArrayAssignment = il.DefineLabel();
		//		Label loopStart = il.DefineLabel();
		...
		//		il.Emit(OpCodes.Ldloc, layer * 2);
		//		il.Emit(OpCodes.Ldfld, fi);
		//		il.Emit(OpCodes.Brfalse_S, loopStart);
		...
		IlEmitCopy = dm.CreateDelegate<Action<T, T>>();
	}
}
