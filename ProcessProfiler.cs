...
namespace ReusableModules.Profiling
{
	...
	public static class ProcessProfiler
	{
		...
		{
			// On machines with high resolution, a "tick" is a hardware-dependent length of time. Stopwatch.Frequency defines
			// how many of these ticks there are per second.
			// On machines without high resolution, a "tick" means 100 ns, so 10,000,000 ticks per second or 10,000 ticks per ms.
			private static readonly double TICKS_PER_SECOND = Stopwatch.IsHighResolution ? Stopwatch.Frequency : 10000000d;
			private static readonly double TICKS_PER_MS = TICKS_PER_SECOND / 1000d;
